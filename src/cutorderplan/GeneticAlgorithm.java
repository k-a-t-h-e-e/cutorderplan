...
public class GeneticAlgorithm {
    ...
    public double calcFitness(Individual individual) {
        ...
        if (Arrays.stream(b).min().getAsInt() <= Main.hmax)
            individual.setB(Arrays.stream(b).min().getAsInt());
        else
  ...
  //      System.out.println(individual.toString() + " " + bmin + " " + fitness);
    ...
    public boolean isTerminationConditionMet(Population population) {
        ...
        if(this.generation>80&&population.getFittest(0).getB()>=Main.hmin)
            return true;
...
*/
    ...
    public Individual[] uniformCrossover(Individual parent1,Individual parent2) {
        ...
        for (int geneIndex = 0; geneIndex < parent1.getChromsomeLength(); geneIndex++) {
            ...
            if (0.5 > Math.random()) {
                if(Arrays.stream(offsprings[1].getChromosome()).sum()-parent2.getGene(geneIndex)+parent1.getGene(geneIndex)<=Main.gmax)
                offsprings[1].setGene(geneIndex, parent1.getGene(geneIndex));

            } else {
                if(Arrays.stream(offsprings[0].getChromosome()).sum()-parent1.getGene(geneIndex)+parent2.getGene(geneIndex)<=Main.gmax)
                offsprings[0].setGene(geneIndex, parent2.getGene(geneIndex));
